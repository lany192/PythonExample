...
model = keras.Sequential([
    ...
    # Hidden layer with 128 densley connected neurons using relu for activation
    keras.layers.Dense(10, activation=tf.nn.softmax)
    # Output layer with 10 possible outputs using softmax (sigmoid) activation
])

# Model configuration of optimizer, loss function and metric of the neural net
model.compile(
    optimizer=tf.train.AdamOptimizer(),
    # Modified version SGD which implements adaptive learning rates for each weight instead of one for all
    loss='sparse_categorical_crossentropy',
    # Our targets are represented as integers so we use 'sparse_categorical_crossentropy'
